...
countdown_label = ttk.Label(status_frame, textvariable=countdown_text, style="Countdown.TLabel")
countdown_label.pack(pady=5,padx=5)
#主程式
def update_countdown():
    if recording:
        current = time.time()
        elapsed = current - start_time
        interval = capture_interval - (elapsed % capture_interval)
        countdown_text.set(f"下次截圖: {interval:.1f}秒")
...
def toggle_recording():
    global recording, start_time, capture_interval, frames
    if not recording:
